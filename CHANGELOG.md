...
* Added [Custom License Namespaces](chapters/appendix-X-custom-license-namespaces.md) which defines a format for custom license identifiers (using the existing `LicenseRef-` prefix) to be specified, for optional use with licenses that are not on the SPDX License List.
